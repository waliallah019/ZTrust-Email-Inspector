...
import { initiateSignup, verifySignup } from '../api';
import { 
...
const SignupForm = () => {
  ...
  const [securityLevel, setSecurityLevel] = useState(0); // Password strength
  ...
  // Initialize dark mode and entry animation
  useEffect(() => {
    ...
    // Interval for security level animation (can be removed if not needed on signup)
  ...
  const handleInitiateSignup = async (e) => {
    ...
    try {
      ...
      setError(err.message || 'Signup initiation failed. Please try again.');
    } finally {
      setIsLoading(false);
  ...
  const handleVerifySignup = async (e) => {
    ...
    try {
      await verifySignup(tempData.email, tempData.password, otp);
  ...
  return (
    <div className={`relative min-h-screen flex flex-col items-center justify-center py-12 px-4 sm:px-6 lg:px-8 transition-colors duration-500 overflow-hidden ${darkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-900'}`}>
      ...
      {/* Branding */}
      <div className={`relative z-10 flex flex-col items-center mb-8 transform transition-all duration-700 ${animateSignup ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
      ...
      <div
        ...
        className={`relative w-full max-w-md transform transition-all duration-700 ${animateSignup ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}
      >
        <div className={`relative ${
          darkMode
        ...
        >
          ...
          <div>
            ...
            <p className={`text-center text-sm mb-6 ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
              {otpSent
                ...
                : 'Join our platform to protect your inbox'}
            </p>
          </div>
          ...
          {!otpSent ? (
            // Initial signup form
            <form className="space-y-6" onSubmit={handleInitiateSignup}>
              <div className="group">
              ...
              <div className="py-4">
                <div className="text-center mb-6">
                  ...
                  <p className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                    ...
                    <br />
                    <span className="font-medium">{tempData?.email}</span>
                  </p>
                </div>
                ...
                <div className="mt-6">
                  ...
                  {/* Placeholder for OTP timer (can implement if needed) */}
